...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\Lenovo\\Downloads\\data anreg\\DATA Persentase Pekerja Anak (usia 10-17 tahun) Menurut Provinsi, 2023.xlsx")
dt
```

```{r}
y <- dt$"Persentase Pekerja Anak (usia 10-17 tahun) Menurut Provinsi (Persen)"
x <- dt$"PDRB"
n <- length(dt)
```
...
$$E[\hat{Y}] = \hat{Y} = 1.86241 + 0.13651 X$$
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 7.575 dan JKG model bernilai 72.948

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 2.0256. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{2.025607} \approx 1.423238$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 1.862412 dan 0.1365132. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Jika dalam uji hipotesis kita ingin menguji apakah $\beta_0$, maka nilai th dapat dihitung sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [0.22604 ,  3.498785] dan untuk parameter $β_1$ adalah [-0.04483777 ,  0.3178641]
...
Misalkan kita ingin menduga nilai rataan (harapan) amatan ketika nilai x=12. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=12 adalah 3.500571 Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 2.372198 hingga 4.628943.
...
Misalkan kita ingin menduga nilai individu amatan ketika nilai x=12. Kita dapat memanfaatkan fungsi `predict` untuk mendapatkan nilai dugaan dan selang kepercayaannya sebagai berikut.

```{r}
predict(model, amatan.diduga, interval = "prediction")
```

Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=12 adalah 3.500571 Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 0.4013964 hingga 6.599745.