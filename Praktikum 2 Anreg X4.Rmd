...
# Data
## Membaca Data
Jika data berupa csv, maka tidak perlu pemanggilan `library(readxl)` karena R
sudah memiliki fungsi bawaan terhadap hal tersebut. Apabila file .csv pada
software excel ditunjukkan dengan delimiter “,” maka dapat menggunakan fungsi
`read.csv`, apabila delimiter berupa “;” maka dapat menggunakan fungsi
`read.csv2`.
```{r}
library(readxl)
data <- read_excel("D:\\NSC-2025.xlsx", sheet="Sheet2")
head(data) #menampilkan sebagian data awal
```
## Menyesuaikan Peubah
Untuk mempermudah pemanggilan peubah pada saat eksplorasi dan analisis, jika
pada file masih mengggunakan nama peubah yang panjang, maka bisa diganti dengan
menggunakan fungsi `colnames()`. Kolom wilayah juga dapat dihapus dengan
menggunakan syntax `namadata[,-1]` dengan -1 menunjukkan bahwa kolom pertama
ingin dihapus.
```{r}
...
n <- nrow(data)
```
Saat ini terdapat 1 peubah Y dan 9 peubah X, misal ingin dilakukan eksplorasi
dan analisis terkait peubah X4
```{r}
plot(data$X4, data$Y,
     main = "Persentase Akses Sanitasi Layak vs Indeks Pembangunan Manusia",
     xlab = "ASL",
     ylab = "IPM")
```
```{r}
...
$$\hat{Y} = 60.25436 + 0.1828016 X_4$$
dengan Y adalah persentase penduduk miskin dan X4 adalah persentase akses sanitasi layak.

Didapatkan nilai $\hat\beta_0 = 60.25436$ artinya nilai dugaan rataan persentase akses sanitasi layak bernilai 0 persen adalah 60.25%, akan tetapi nilai 0 tidak ada dalam selang pengamatan rata-rata lama sekolah sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut.

Nilai $\hat\beta_0 = 0.1828016$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika persentase akses sanitasi layak berubah satu tahun adalah sebesar 0.18%.

Koefisien determinasi yang dihasilkan adalah 0.239335 atau 23.93% artinya keragaman persentase penduduk miskin yang dapat dijelaskan oleh persentase akses sanitasi layak adalah sebesar 23.93%, sisanya yaitu 76.07% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.